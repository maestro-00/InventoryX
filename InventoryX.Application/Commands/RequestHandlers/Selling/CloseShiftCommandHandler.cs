...
public sealed class RecordCashMovementCommandHandler(IAppDbContext context, ITenantContext tenantContext, IPosAccess posAccess) : IRequestHandler<RecordCashMovementCommand, CashMovementDto>
{
    public async Task<CashMovementDto> Handle(RecordCashMovementCommand request, CancellationToken cancellationToken)
    {
        ...
        var shift = await context.Shifts.SingleOrDefaultAsync(item => item.Id == request.ShiftId && item.Status == ShiftStatus.Open, cancellationToken)
            ?? throw new ConflictException("Cash movement requires an open shift.");
...
public sealed class CloseShiftCommandHandler(IAppDbContext context, ITenantContext tenantContext, INotificationService notificationService, IPosAccess posAccess) : IRequestHandler<CloseShiftCommand, ShiftDto>
{
