...
public record SyncProductDto(Guid Id, string Name, string? Sku, string? Barcode, decimal SellingPrice, Guid? TaxTreatmentId, string Version);
...
public record SyncSnapshotDto(string Watermark, Guid RegisterId, Guid LocationId,
    List<SyncProductDto> Products, List<SyncVariantDto> Variants, List<SyncTaxDto> TaxTreatments, List<SyncStockDto> Stock);
