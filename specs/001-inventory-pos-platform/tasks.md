...
- [X] T077 [P] [US5] GET /billing/plans and GET /billing/subscription (status, usage vs limits) per contracts/billing.md in InventoryX.Application/Queries/.../Billing/ + InventoryX.Presentation/Controllers/v1/BillingController.cs
- [X] T078 [US5] Upgrade (immediate, pro-rata charge) / downgrade (period-end, over-limit acknowledgement) / cancel / reactivate commands in InventoryX.Application/Commands/.../Billing/
- [X] T079 [US5] Payment-method initialization endpoint (card | mobile_money + provider + msisdn) in BillingController + gateway wiring
- [X] T080 [US5] Paystack webhook endpoint: signature check, idempotent by event id, drives charge success/failure into state machine in InventoryX.Presentation/Controllers/v1/PaystackWebhookController.cs
...
- [X] T088 [US6] Variance-above-threshold manager notification + UnusualVoids detection hook in InventoryX.Application/Commands/RequestHandlers/Selling/CloseShiftCommandHandler.cs
- [X] T089 [US6] Z-report query + GET /shifts/{id}/z-report in InventoryX.Application/Queries/.../Selling/ + ShiftsController per contracts/pos-sales-sync.md
- [X] T090 [US6] Registers CRUD with plan cap + concurrent-shift prevention + sale-requires-open-shift enforcement finalized in InventoryX.Presentation/Controllers/v1/RegistersController.cs + validators
...
- [X] T092 [P] [US7] Failing tests for FEFO batch selection at sale + batch-required validation for batch-tracked products in tests/InventoryX.Application.Tests/Inventory/FefoIssueTests.cs
- [X] T093 [P] [US7] Failing tests for receipt of short/over/damaged deliveries (order stays open or closes short with reason) in tests/InventoryX.Application.Tests/Purchasing/GoodsReceiptTests.cs
...
- [X] T096 [P] [US7] PO PDF generation + email-to-supplier send in InventoryX.Infrastructure/Services/PurchaseOrderPdfService.cs + send endpoint
- [X] T097 [US7] Batch entity + GoodsReceipt flow (batch number + expiry capture, ledger Receipt movements, PO Partially/FullyReceived updates, close-short) in InventoryX.Domain/Models/Inventory/Batch.cs + InventoryX.Domain/Models/Purchasing/GoodsReceipt.cs + InventoryX.Application/.../Purchasing/
...
- [X] T102 [US7] AlertScanWorker BackgroundService raising via the Foundational Notification entity/service from T014 (low/out-of-stock, expiry horizon, overstock, slow-moving; consolidation by key) in InventoryX.Infrastructure/BackgroundJobs/AlertScanWorker.cs
- [X] T103 [P] [US7] Supplier performance metrics (on-time rate, achieved lead time, price history) derived query in InventoryX.Application/Queries/.../Purchasing/SupplierPerformanceQueryHandler.cs
...
- [X] T105 [P] [US8] Failing tests for Ghana tax report totals by rate/levy and period in tests/InventoryX.Application.Tests/Reports/TaxReportTests.cs
...
- [X] T121 CRITICAL Add success/failure tests for report-schedule deletion and lookup plus an integration-level weekly scheduled-email worker test per Constitution II and US8/AC3 (partial)
- [X] T122 CRITICAL Replace hand-written ReportSchedule DTO mapping with an AutoMapper profile used by schedule command/query handlers per Constitution IV (contradicts)
- [X] T123 CRITICAL Add bounded pagination to GET /reports/schedules with request/response contract tests per Technology & Security Constraints (contradicts)
...
- [X] T126 Remove unused AutoFixture infrastructure and its Fare/NETStandard.Library dependency chain so System.Net.Http 4.3.0 and System.Text.RegularExpressions 4.3.0 no longer appear in the test package audit per Constitution V (partial)
- [X] T127 Replace the vulnerable bundled SQLitePCLRaw.lib.e_sqlite3 test dependency with a non-vulnerable relational test provider or patched native package, preserving relational test coverage and a clean package audit per Constitution V (partial)
